...
### Changed
- `prd-gen` (v0.4.0) - PRD 章节结构调整
  - **移除第 8 章（项目计划）** - 不再生成里程碑/时间表部分，由开发团队评估工期
  - **新增第 7 章（合规建议）** - 合规部分包含：
    - 7.1 合规要点概述
    - 7.2 风险提示（隐私合规/内容合规/数据安全/第三方合规）
    - 7.3 合规报告附件
  - **章节重新编号**：
    - 原型设计：第 7 章 → 第 8 章
    - 成功指标：保持第 9 章
  - **更新第 6 章（项目风险与应对）** - 明确标注仅包含产品/业务风险，不含技术实现风险
  - **更新质量标准**：从"9 个必需章节"改为"8 个必需章节（不含项目计划）"
  - **更新质量门控清单**：
    - 移除"项目计划 with timeline"
